<?php
 include 'process/init.php';
$autenticado = $_SESSION['autenticado'] ?? false;
if($autenticado !== true) {
	header('location: index.php');
	exit();
} 
?>

<!DOCTYPE html>
<html lang="pt-BR">
	<?php include "components/header.php" ?>
	<body>
		<?php include "components/menu-top-logado.php" ?>
	
	
		<section class="book-box">
			<a href="exemplo1.php" class="book">
				<img class="book__img" src="http://4.bp.blogspot.com/-uO59jBa8yGg/WBqFrzbAYyI/AAAAAAAADN0/3hp-nItF3Ds9NNwCg7M9uq77bMkpYp0kwCK4B/s1600/2-1280x853.jpg" alt="">
				<div class="book__paragraph">
					<h1 class="heading-logo heading-logo--white">Por Lugares Incríveis</h1>
				</div>
			</a>

			<a href="exemplo2.php" class="book">
				<img class="book__img" src="https://resenhasalacarte.com.br/wp-content/uploads/2018/06/pequeno-principe-livro-frases2.jpg" alt="">
				<div class="book__paragraph">
					<h1 class="heading-logo heading-logo--white">O Pequeno Princípe</h1>
				</div>
			</a>

			<a href="exemplo3.php" class="book">
				<img class="book__img" src="https://www.elefantevoador.com/wp-content/uploads/2017/11/05-3-650x450.jpg" alt="">
				<div class="book__paragraph">
					<h1 class="heading-logo heading-logo--white">Extraordinário</h1>
				</div>
			</a>

		</section>
	

		<section class="hero">
			<p> “A doação é um exercício de desapego.<br /> Ela deve beneficiar mais quem doa do<br /> que quem recebe.”</p>
		</section>


	<?php include "components/footer.php" ?>
	<script src="js/scrollMenuEfect.js"></script>
	</body>
</html>
